...
7. **Start the server** (if not running): `python app/server.py --host 127.0.0.1 --port 8001`
8. **Publish**: `python agent/publish_results.py research_runs/<run_id>.json --server http://127.0.0.1:8001` (writes `data/.last_run_papers.json` for next-run dedup + triggers gh-pages deploy).
9. **See it on the page**: open `http://127.0.0.1:8001/`. Tag-filter chips + compact cards; source_tier priority + score sort; click a title for the detail page (short summary + tags + source link). `GET /api/papers` returns only the latest run.
10. **Close the loop**: update `data/.last_run` to this research time (ISO 8601). Fold this week's mistakes into `AGENTS.md` lessons, `validation-rules.md`, and `research-prompt.md`. Run `python tests/test_research_pipeline.py`, `tests/test_build.py`, `app/gates/gate_all.py`.
