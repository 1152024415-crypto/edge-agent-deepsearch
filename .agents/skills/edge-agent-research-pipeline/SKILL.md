...
9. **Collect the separate community radar**: search the past seven calendar days across X, Bluesky, Reddit, Hacker News, Mastodon, GitHub Discussions, Hugging Face Discussions/Models/Spaces, YouTube/Bilibili and vendor forums; write `data/community_radar.json`. Record `found/no_match/limited/unavailable` plus a note for all nine sources. Use only directly openable, date-verifiable public X posts; record `limited` instead of inventing completeness. Videos must come from official channels or link back to first-party projects. Community discussion URLs never enter formal `paper_url`; promotion requires a first-party source and the full formal contract.
