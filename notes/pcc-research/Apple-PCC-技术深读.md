...
### 核心思路：把"别做坏事"从政策变成"技术上不可行"

传统云服务的隐私靠"我们承诺不存你的数据"这类规章。PCC 的思路是：政策能被违反，那就让违规在技术上发生不了——

- 想长期存数据？生产节点不提供可用于保留用户请求的持久化路径；临时数据卷的密钥重启即丢，等于密码学擦除。
...
3. **没法盯上特定的人** —— 攻击者要偷某个特定用户的数据，必须广泛破坏 PCC 的路由、节点选择和足够多节点，单点攻破很难定向到某个人（不可定向攻击，non-targetability）。
...
为达成这五条，PCC 把六个技术组件拼成一条端到端链路：让请求**先匿名**（经第三方中转，让 PCC 请求路径看不到真实 IP 与账号的关联）、再**授权但不关联身份**（盲签名）、然后**远程核对服务器在跑什么代码**（远程证明）、把密钥锁进**专用硬件**（Secure Enclave）、把软件版本挂在**公开防篡改日志**上（透明日志）、最后用**不可篡改的启动链**锁死攻击面（安全启动）。每个组件的细节见第 5 节。
...
## 1. 背景与动机
![alt text](image.png)
消费者嘴上说在乎隐私，但照样把数据交给 ChatGPT 这类服务；企业（医疗、fintech、legal）则**真的**在乎，因为数据泄露有直接的财务后果。近年来的一系列数据泄露事件（如 2025 年 DeepSeek 暴露超百万行日志/敏感记录）会持续发生，而在 AI 时代被进一步放大——因为**数据训练、数据归属、谁从中取走价值**成了新焦点。

企业的传统做法是为隐私付费：单租户、on-prem、bring-your-own-cloud。但 **AI 让这条路变难**——没有中型企业或创业公司会去自购一集群 H100 自建推理栈。安全变得更贵了。

这正是 Apple 要解决的：让消费者用上 AI，但手机端算力不够，而**隐私是 Apple 的核心卖点**。Apple Intelligence 与 PCC 在 2024/06 正式宣布，2024/10 起随 iOS 18.1 等版本进入首批可用阶段；PCC 是其云端那一半。只要你让客户交数据，就在让渡隐私；PCC 的目标是把这种让渡降到技术意义上的零。
...
2. **可强制执行的保证（Enforceable guarantees）** —— 一切由**代码**强制，而非约定或策略。注解：不想让人 SSH 进机器看数据，就别装 SSH；不想让数据被长期保留，就不提供持久化落盘路径，并让临时数据卷密钥重启即丢。

3. **不可定向攻击（Non-targetability）** —— 攻击者无法在不"广泛攻破"的前提下，定向窃取特定用户的数据。换言之，不是攻下一台节点就能盯上某个人，而是要同时破坏请求路由、节点选择和足够多 PCC 节点。
...
- **enforceable + stateless 的具体体现**：不提供可长期保留用户请求的持久化路径；临时数据卷的加密密钥在启动时随机生成且不持久化，重启后**永远恢复不出来**。
...
OHTTP 是一个标准化的 HTTP 隐私扩展，和 Tor 一样用到了"拆分信任"的思想，但范围窄得多：它只把源 IP 与请求内容解耦，不是通用匿名网络。
...
- Cloudflare / Fastly 提供这种 relay / IP 解耦服务。
- **关键设计**：relay 与 PCC 网关不能由同一信任方同时掌握，否则双方信息一拼就可能还原用户身份。整个机制的目的是"提高门槛"——把一部分信任押在 Cloudflare/Fastly 的独立性和声誉上（它们不向 Apple 交代真实来源），而 Apple 自己也不想知。
...
在 PCC 里，盲签名用于 **OAuth 授权环节**：让 AI 节点能确认"这是一个合法请求"，但**不知是谁发的**。约束：签名证书/参数不能细分到可识别用户的粒度——否则给每个用户单独签一套，就能反查。
...
通用 remote attestation 里，要 attest 的常常不止"软件版本"，而是一长串硬件与启动状态：CPU/TEE、GPU、内存加密、TPM/vTPM、bootloader、OS、驱动和应用度量等。原始 Apple Silicon PCC 更强调 Secure Boot、Code Signing、trust cache、节点身份和软件镜像度量；2026 扩展到 Google Cloud 后，Intel TDX、NVIDIA confidential GPU 与 Google Titan 才更直接进入这条证明链。
...
**Secure Enclave 是 Apple 的硬件安全子系统**：一块专用硬件、独立小芯片、跑独立 OS。PCC 相关的请求解密密钥材料由它保护，目标是**不可复制、不可提取**；它是信任根之一，而不是"所有系统私钥都在这里"。
...
机制很简单：开发者发布二进制时，把 measurement/hash 写进这条公开 log，声明"我是 Bob，发布这个二进制"。用户拿到二进制 → hash 不匹配 log → 知道系统被入侵（不是官方发布的）。只有在源码、构建脚本和可复现构建链也公开时，这个 hash 才能进一步可靠地链到源码。

在 PCC 里：Apple 每次 PCC 更新都把**所有二进制的 hash** 发到这条 log；researcher 逐个审查"这二进制确实在做 Apple 声称的事"；iPhone 收到节点 claims 时，因为有 log 背书而信任。这就是 **offline vs online**：离线验证 + 在线核对 attestation 是否匹配公开 log。关键不是物理上只能有一条 log，而是必须有一个全局一致、可审计、append-only 的公开发布视图；否则就会出现 split-view / 定向发布风险。
...
- **信任单点**：根信任、硬件设计、密钥生成、编译/发布流程仍由 Apple 控制。透明日志能证明"线上跑的是公开发布版本"，但不能证明公开版本本身没有漏洞或后门。Apple 有强烈激励做对，系统也确实 impressive，但这仍是一处结构性单点。
- **源码不全公开**：Apple 公开了部分源码、二进制镜像、Virtual Research Environment 和研究接口，但没有给出完整源码与完整可复现构建链，因此外部审查深度受限。
- **开放边界仍在 Apple 生态内**：截至 2026/07，符合条件的 App Store 开发者可以通过 Apple Foundation Models/PCC entitlement 使用 PCC，但它仍不是任意云 API、任意设备或任意第三方模型的开放平台。

### 7.2 权衡

- **低延迟 AI workload 拿不到**：复杂请求仍要回 Apple/PCC 云端路径；端侧模型优先，但实时性强的 workload 会受网络与 attestation 链路影响。
- **运维极复杂**：不能 SSH、看不到在干嘛 → 几乎没法 debug。
- **计算成本更高**：OHTTP、盲签名、远程证明、透明日志核对和多层加密都会带来额外工程与计算开销；不要轻易写死"几层加密"。
- **无法做细粒度用量追踪**：全匿名 → 不能按会破坏匿名性的方式追踪"这个用户这次发了什么、花了多少钱"。Apple 仍可做匿名凭证、daily limits、fraud/abuse 检测，但不等同 full usage tracking。
- **能力边界明确**：不能自定义模型、不能 fine-tune；第三方开发者访问是受资格、entitlement、设备/系统与额度限制的 Apple 生态内能力。
...
如果你（非 Apple）要做类似保证，路径通常是**用云厂商的 confidential VM**（带 vTPM）再叠加自己的远程证明、透明发布和请求匿名化。需要 GPU 的 confidential VM 时，公开成熟度主要集中在 Azure / GCP 路径；AWS 的机密计算原语和 GPU 组合路径不同，落地前需要按当期产品文档单独核实。
...
- Security research on Private Cloud Compute（2024/10，源码/VM/虚拟研究环境）：https://security.apple.com/blog/pcc-security-research/
- Expanding Private Cloud Compute（2026/06）：https://security.apple.com/blog/expanding-pcc/
- Accessing Private Cloud Compute（Apple Developer，2026 开发者访问条件）：https://developer.apple.com/private-cloud-compute/
- Build with the new Apple Foundation Model on Private Cloud Compute（WWDC26）：https://developer.apple.com/videos/play/wwdc2026/319/
...
**外部案例与竞品**
- Wiz Research on DeepSeek exposed database（2025/01）：https://www.wiz.io/blog/wiz-research-uncovers-exposed-deepseek-database-leak
- OpenAI response to NYT data demands（2025/06，2025/10 更新）：https://openai.com/index/response-to-nyt-data-demands/
- WhatsApp Private Processing（Meta，2025/04）：https://engineering.fb.com/2025/04/29/security/whatsapp-private-processing-ai-tools/
