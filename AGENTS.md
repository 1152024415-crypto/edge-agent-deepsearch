...
- [2026-07-30] github trending 每次要手动跑——gate_release `check_trending_freshness` 只拦过期不自动刷，`collect_github_trending.py` 靠主 agent 手动跑（gate FAIL 才补）。根因：trending 刷新不在 auto-deploy 流程里。修复：`agent/refresh_trending.py`（collect + 转 top20 一步）加进 server `_deploy_to_ghpages`（build.py 前跑），每次 publish 自动刷 trending。**该自动化的步骤不许靠人记——进 deploy 流程，gate 只兜底（刷新失败仍能拦）。**
