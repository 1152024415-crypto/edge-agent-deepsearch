...
5. **社区雷达**：X、Bluesky、Reddit、Hacker News、Mastodon、GitHub Discussions、Hugging Face、YouTube / Bilibili、厂商论坛的 7 日线索。显示来源覆盖、中文总结、价值判断、设备范围和核验状态；明确“不等同于正式周报”。
...
发布门 `gate_release.py` 必须在真实 `site/index.html` 上检查推荐、本周判断、完整资料库、来源构成、社区雷达、GitHub 发现线索的存在与顺序；校验九类社区来源覆盖、内联快照与源数据一致，并阻止社媒讨论 URL 混入正式 papers 或完整资料库再次使用 `!isRecommended` 排除推荐条目。
