...
13. **首页字段人类可读**：`abstract`/`effects`/`mechanism` 用中文短句给人看（这是什么/有什么结果/怎么做到的），每段 1-2 句，避免论文腔。`effects` 必须来自原文，没有报告写 `未报告`，不许编造。
14. **不凑数**：本周合格内容不足就少收，不拿学术充大厂，不拿不确定链接凑数。
...
按 `docs/agent-guide/output-contract.md` 的 JSON 结构输出，不要输出 markdown 表格、不要 1-5 分评分。每条含：`id`/`title`/`abstract`/`effects`/`mechanism`/`paper_url`/`date`/`score`+`score_relevance`+`score_contribution`/`source_tier`/`open_source`/`tags`/`score_reason`/`authors`/`vendors`/`venue`/`recommendation`。子 agent 只产 JSON，不改代码、网页、服务器。
