...
29. **社区雷达隔离门**：`data/community_radar.json` 必须覆盖以运行日为末日的 7 个自然日，并为 X / Reddit / Hacker News / 厂商论坛 / 开发者论坛逐一写 `found` / `no_match` / `limited` / `unavailable` 与说明。条目必须使用可打开的讨论直达 URL、可核验日期、中文名称/总结/价值判断、设备范围和核验状态。X 未登录公开检索不足时写 `limited`，不得用论坛转述冒充 X 原帖。社媒/论坛 URL 不得进入正式 `paper_url`；晋升正式周报必须重新回链一手材料并满足正式来源契约。`gate_release` 校验源数据、内联周快照和正式/社区层隔离。
