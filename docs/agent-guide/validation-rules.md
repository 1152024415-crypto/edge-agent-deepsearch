...
自动校验覆盖：必填字段、中文 `abstract`、推荐条目的中文 `title_zh` 和 `recommendation_reason`、内部占位词、`source_tier` 枚举、`tags` 词表、`date` 7 天窗口、`score`=2 维之和、官方域名、github URL、vendors 非空、**死链检查**、**arXiv date 核对**、跨 run 去重 warning。
